...
function registerNodeEl(id, el) {
  ...
  if (dom) {
    ...
    // Keep observation attached even when ref callback is re-fired with same DOM.
    // This is important when refs run before nodeRO is created in onMounted.
    nodeRO?.observe(dom)
...
onMounted(() => {
  ...
  // Re-attach observation for nodes whose refs were registered before onMounted.
  for (const dom of nodeElMap.values()) nodeRO.observe(dom)
